...
Personas are defined as Markdown files in `~/.agents/ccpersona/personas/`. Existing files under `~/.agents/personas/` or `~/.claude/personas/` are read as migration fallbacks, but new files should live in `~/.agents/ccpersona/personas/`. Each file follows this structure:
